...
function createMesh(geometry){
    var meshGeometry = geometry || new THREE.TorusKnotGeometry();
    var materials = [
...
var controls = new function(){
    ...
    this.redraw = function(){
        scene.remove(mesh);
        mesh = createMesh(new THREE.TorusKnotGeometry(controls.radius, controls.tube,
            controls.radialSegments, controls.tubularSegments, controls.p, controls.q, controls.heightScale));
        scene.add(mesh);
    }
}
...
gui.add(controls, 'p', -50, 50).onChange(controls.redraw);
gui.add(controls, 'q', -50, 50).onChange(controls.redraw);
